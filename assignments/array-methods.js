...
// I would actually rather use map() for this
const fullName = [];
...
const largeShirts = runners.filter(x => (x.shirt_size === 'L'));
console.log(largeShirts);


// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
const ticketPriceTotal = runners.reduce((acc, e) => acc + e.donation, 0);
...
// Problem 1
// create a list of runner's full names for runners from a specified company
const company = 'Skinix';
const company_runners = runners.filter(x => (x.company_name === company)).map(x =>
	`${x.first_name} ${x.last_name}`);
console.log(company_runners);


// Problem 2
// log a sentence about each runner who donated more than a certain threshold.
const big_donation = 200;
runners.filter(x => (x.donation >= big_donation)).forEach(x => 
	console.log(`Special thanks to ${x.first_name} ${x.last_name} who donated $${x.donation}!`));


// Problem 3
// calculate the average donation
let avg_donation = runners.reduce((acc, e) => acc + (e.donation / runners.length), 0);
avg_donation = Math.round(avg_donation * 100) / 100;
console.log(`The average donation was $${avg_donation}`);
